...
export default function main() {
    const canvas = document.querySelector('#voxel-demo');
    ...
    function generate_terrain() {
        ...
        const texture = loader.load('../data/flourish-cc-by-nc-sa.png', render);
        ...
        for (let cz = 0; cz < cellCount; ++cz) {
            for (let cx = 0; cx < cellCount; ++cx) {
                for (let y = 0; y < cellSize; ++y) {
                    for (let z = cz; z < cz+cellSize; ++z) {
                        for (let x = cx; x < cx+cellSize; ++x) {
        ...
        function inputHandler() {
            ...
            function placeVoxelIfNoMovement(event) {
                if (mouse.moveX < 20 && mouse.moveY < 20) {
                    placeVoxel(event);
                }
